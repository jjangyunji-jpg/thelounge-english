...
### 더블카운트 정책
- **보강 (예정+완료)**: 보강 세션(`reschedule_origin_dates`에 다른 날짜 포함, 취소 카테고리 없음)이면 완료 여부와 무관하게 **보강 +1**. 완료 시 **완료 +1**까지 별도 카운트.
...
- **병결 + 보강 매칭**: 병결(`sick`)은 항상 +1. 같은 학생의 보강 세션 중 origin에 그 병결 KST 날짜가 포함되면 보강 카운트로 매칭됨. 매칭되는 보강이 없으면 셀 옆에 ⚠ 아이콘 표시 (`sick_unmatched`).
