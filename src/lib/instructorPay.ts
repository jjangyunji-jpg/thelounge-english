...
export function calcSessionPay(
  ...
  // 일반 강사 — 신규 규정
  if (isNewRules) {
    // 무급: 강사 취소(병결 포함), 사전 취소 — 보강 진행 시 별도 보강 세션에서 정상 지급
    if (ct === "instructor_cancel" || ct === "sick" || ct === "advance_cancel") {
      return { included: false, payPerHour: 0, noteSuffix: "" };
    }
    // 학생 취소 (48h~4h 전) → BASE 11,000
    if (ct === "student_cancel" || ct === "late_cancel") {
      return { included: true, payPerHour: BASE_PAY, noteSuffix: " [학생 취소 — 기본급만]" };
    }
