...
export async function exportStudentGuidePdf() {
  ...
  doc.addPage();
  y = 25;
  y = drawHeader(doc, y, "1. 처음 시작하기 (회원가입)");
  y = drawBody(doc, y, [
    "웹사이트에 접속하여 직접 회원가입을 진행합니다.",
    "가입 후 관리자의 승인을 받으면 로그인이 가능합니다.",
  ]);
  y = drawSubHeader(doc, y, "회원가입 단계");
  y = drawNumberedSteps(doc, y, [
    "웹사이트 로그인 화면에서 '회원가입' 링크를 클릭합니다.",
    "이름, 이메일 주소를 입력합니다.",
    "비밀번호를 설정합니다. (8자 이상)",
    "비밀번호를 한 번 더 입력하여 확인합니다.",
    "'회원가입' 버튼을 클릭합니다.",
    "관리자가 가입을 승인하면 로그인이 가능해집니다.",
  ]);
  y = drawTip(doc, y, "승인 전까지는 로그인이 불가합니다. 승인이 완료되면 로그인해주세요.");
