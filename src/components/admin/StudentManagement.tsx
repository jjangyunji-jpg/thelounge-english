...
import { Plus, Search, Download, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, UserX, BookOpen, Edit2, RefreshCw, Trash2, Target, Check, X, Bell, BellOff, Video, ExternalLink, Link2, PenLine, Mic, Brain, Clock, Mail, Loader2, FileText, Paperclip, Monitor, Pause, Play, Users, ArrowRightLeft, Undo2 } from "lucide-react";
...
export default function StudentManagement() {
  ...
  const propagateMeetLinkToSessions = async (
    ...
    // 이관 레코드는 이관일 이후 세션의 source of truth이므로 stale instructor_name 세션까지 보정
    const fromDate = transferDate || startDate;
    ...
    if (!transferDate) query = query.eq("instructor_name", instructorName);
  ...
  const deleteMeetLink = async (studentId: number) => {
    ...
    if (student?.dbId) {
      ...
      const startDate = (student as any).startDate || null;
      const transferDate = (student as any).transferDate || null;
      await propagateMeetLinkToSessions(student.name, student.instructor, null, startDate, transferDate);
    }
    setStudents((prev) =>
